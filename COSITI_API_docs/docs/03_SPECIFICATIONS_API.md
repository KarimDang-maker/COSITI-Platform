...
| POST | `/droits/adherents/{id}/recalculer` | Réservé `DAF`, `SUPER_ADMIN`, motif obligatoire |
...
## 9. Relances, notifications, reporting, administration
...
| GET | `/tableaux-de-bord/direction` · `/daf` · `/cnps` · `/terrain` |
...
## 10. Contrats en attente de validation — ne pas coder avant accord du chef de projet

Le modèle de rôles V1 (`CHEF_AGENT_TERRAIN` avec privilèges de supervision, contrôle DAF revu) fait apparaître des besoins que le contrat REST existant ne couvre pas encore. **Ces chemins ne doivent pas être inventés par l'agent de code** : ce sont des propositions à valider, pas des endpoints à implémenter.

| Besoin | Piste de chemin [A] | Description |
|---|---|---|
| Désignation d'un agent comme Chef des agents de terrain | `POST /agents/{id}/designer-chef` | Réservé DGA. Ajoute les privilèges de supervision à un agent existant, sans créer de nouvelle entité. Opération journalisée (`AGENT_DESIGNATION_CHEF`) |
| Attribution d'un objectif à un agent ou une zone | `POST /agents/{id}/objectifs` ou `POST /zones/{id}/objectifs` | Réservé Chef/DGA selon périmètre. Le choix agent vs zone comme porteur de l'objectif reste à trancher |
| Confirmation hiérarchique d'une collecte | `POST /paiements/{id}/confirmer-chef` | Réservé au Chef des agents de terrain de l'agent encaisseur, avant le contrôle DAF. S'intercale entre la création du paiement et `POST /paiements/{id}/valider` |
| Signalement structuré d'une incohérence documentaire/financière | `POST /paiements/{id}/signaler-incoherence` | Réservé DAF/Chef, motif obligatoire, ne supprime rien, bascule le paiement dans un état distinct de `ANNULE` |
| Remise interne caisse/coffre-fort | Révision de `/remises-caisse` existant | À confirmer : le processus actuel (déclaration par l'agent, réception par un tiers) couvre-t-il le nouveau scénario DAF de S05, ou faut-il un objet distinct ? |

Tant qu'un contrat n'est pas validé, l'écran ou le service correspondant reste marqué comme non disponible plutôt que de simuler un comportement.
...
| `/actuator/info`, `/metrics`, `/prometheus` | Authentifié, rôle `SUPER_ADMIN` |
