...
| MFA | TOTP obligatoire pour `PCA`, `DG`, `DGA`, `DAF`, `SUPER_ADMIN` ; recommandé pour les autres [A] |
...
- Le Super Administrateur (`SUPER_ADMIN`) n'a pas d'accès métier courant aux données nominatives ; il administre les comptes et la technique, pas les adhérents ni la trésorerie. Ses accès exceptionnels sont journalisés et notifiés. Il ne devient jamais automatiquement un acteur métier (pas de privilège financier implicite).
