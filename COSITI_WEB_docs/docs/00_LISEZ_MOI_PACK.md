...
Le fichier `CLAUDE.md` à la racine du dépôt regroupe les règles transverses (rôles V1, méthode de sprint, marqueurs [C]/[A]/[V], périmètre exclu). Ce pack en est la traduction technique côté frontend ; en cas de doute, `CLAUDE.md` fait foi sur l'organisation du travail, ce pack fait foi sur le détail technique.
...
**Rôles V1 (huit, exhaustifs)** : `PCA`, `DG`, `DGA`, `DAF`, `GESTIONNAIRE_COMPTE`, `CHEF_AGENT_TERRAIN`, `AGENT_TERRAIN`, `SUPER_ADMIN`. Les rôles `TELECONSEILLER`, `MARKETING`, `RESP_ZONE`, `RESP_CNPS` du prototype React existant (`src/`) sont **supprimés** : ne pas les faire réapparaître dans un composant, un libellé ou une donnée de démonstration.
