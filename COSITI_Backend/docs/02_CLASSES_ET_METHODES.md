...
├── organisation/            Zone, Agent, AffectationPortefeuille, HistoriqueDesignationChef
│                            + services (dont ServiceAgent) + controleurs
...
├── compterendu/             CompteRendu, ServiceCompteRendu `[A]` — chaîne Agent → Gestionnaire → DGA
├── daf/                     RapportDaf, ServiceRapportDaf `[A]` — flux DAF → PCA
...
```
Règles (8 rôles V1, `../Roles des acteurs.md §11`) : `AGENT_TERRAIN` → uniquement les adhérents de son portefeuille ouvert · `CHEF_AGENTS_TERRAIN` → les agents et portefeuilles de son équipe (périmètre exact `[A]`) · `GESTIONNAIRE_COMPTES` → adhérents, CNPS, comptes rendus reçus, sur son périmètre organisationnel · `PCA`, `DG`, `DGA`, `DAF`, `ADMIN_SYSTEME` → global. L'accès de `ADMIN_SYSTEME` aux données nominatives est exceptionnel et journalisé (`04_SECURITE.md §3`) : il n'a pas d'accès métier courant.
...
### `ServiceAgent` — création et désignation `[A]`
Contrat non confirmé, à valider avant codage (jalon J3, `../Roles des acteurs.md §14`).
```java
public interface ServiceAgent {
    AgentDto creerParDga(CreationAgentDto dto, Utilisateur dga);           // DGA-F01, audité AGENT_CREATION_PAR_DGA
    AgentDto affecter(UUID agentId, UUID zoneId, Utilisateur dga);          // DGA-F02
    AgentDto designerChef(UUID agentId, String motif, Utilisateur dga);     // DGA-F03, exige rôle DGA, audité AGENT_DESIGNATION_CHEF
    AgentDto remplacerChef(UUID nouvelAgentId, String motif, Utilisateur dga); // DGA-F04, audité AGENT_REMPLACEMENT_CHEF
    List<HistoriqueDesignationChefDto> historiqueChef(UUID zoneId);
}
```
`designerChef` lève `ExceptionAutorisation` si l'auteur n'a pas le rôle `DGA`. Un seul agent avec `est_chef = true` par périmètre organisationnel (périmètre exact à confirmer avec `[A]`) : `remplacerChef` clôt la désignation précédente dans la même transaction.

### `ServiceCompteRendu` — chaîne Agent → Gestionnaire → DGA `[A]`
Contrat non confirmé, à valider avant codage (jalon J8, `../Roles des acteurs.md §14`).
```java
public interface ServiceCompteRendu {
    CompteRenduDto produire(CreationCompteRenduDto dto, Utilisateur agent);           // Agent → Gestionnaire
    CompteRenduDto controler(UUID compteRenduId, Utilisateur gestionnaire);
    CompteRenduDto consolider(List<UUID> compteRenduIds, Utilisateur gestionnaire);   // Gestionnaire → DGA
    CompteRenduDto transmettre(UUID compteRenduId, Utilisateur gestionnaire);
    ReponsePaginee<CompteRenduDto> recus(CritereCompteRendu critere, Pageable p, Utilisateur demandeur);
}
```
`consolider` ne recalcule aucune règle métier inventée : il agrège les comptes rendus sources tels que structurés, avec traçabilité (`comptes_rendus_sources`).
...
- Un recalcul complet est possible et journalisé (`DROITS_RECALCUL`), réservé à `DAF` et `ADMIN_SYSTEME`.
...
### `ServiceTableauBord`
Six dashboards exclusivement (`../Roles des acteurs.md §2` et `§11`) — **le Chef des agents de terrain et l'Agent de terrain n'ont pas de dashboard dédié**, ne pas ajouter de méthode pour eux.
```java
public interface ServiceTableauBord {
    TableauBordPcaDto pca(CritereTableauBord c, Utilisateur u);
    TableauBordDgDto dg(CritereTableauBord c, Utilisateur u);
    TableauBordDgaDto dga(CritereTableauBord c, Utilisateur u);
    TableauBordDafDto daf(CritereTableauBord c, Utilisateur u);
    TableauBordGestionnaireDto gestionnaire(CritereTableauBord c, Utilisateur u);
    TableauBordSuperAdminDto superAdmin(CritereTableauBord c, Utilisateur u);
}
```
Indicateur central : `tauxActivation = adherentsAyantCotiseAuMoinsUneFois / adherentsEnregistres`. Calculé sur les vues matérialisées, jamais par agrégation à la volée sur l'ensemble des paiements. Le contenu de `gestionnaire()` reprend ce que couvrait l'ancienne méthode `cnps()` (éligibles non immatriculés, dossiers incomplets, déclarations) — la CNPS est un domaine du Gestionnaire des comptes, pas un rôle séparé.
