...
| Comptes de test | Aucun compte par défaut en base **en production**. Le premier administrateur est créé par une commande d'amorçage exigeant un mot de passe fourni. **Dérogation explicite et volontaire, développement/démonstration uniquement, jamais activée en profil prod** : `cm.cositi.api.securite.bootstrap.SeedComptesDemonstrationDev` (`@Profile({"dev","local"})` + vérification explicite en code que `prod` n'est jamais actif — double verrou) crée, au démarrage et de façon idempotente, un compte par rôle (`demo.pca`, `demo.dg`, `demo.dga`, `demo.daf`, `demo.gestionnaire`, `demo.chef`, `demo.agent`, `demo.superadmin`) avec un mot de passe fixe documenté dans `CONNEXION.md` (fichier local non versionné), audité (`UTILISATEUR_CREATION`, auteur `SYSTEME`), à la demande explicite de l'utilisateur pour pouvoir démontrer les 8 tableaux de bord/interfaces. Voir aussi ce composant pour la création d'un agent et d'une zone de démonstration associés (`AGENT_TERRAIN`/`CHEF_AGENT_TERRAIN`) |
...
| Jeton d'accès | JWT signé, durée 15 minutes, algorithme RS256 ou ES256. **`alg: none` et HS256 avec clé faible explicitement refusés**. **Décision de prototype (J1, 2026-09-21)** : l'implémentation actuelle utilise **HS256** avec un secret d'au moins 256 bits, fourni exclusivement par la variable d'environnement `JWT_SECRET` (`cositi.securite.jwt.secret`, jamais de valeur en dur en production — la valeur par défaut de `application.properties` n'est qu'un repli de développement local). `ServiceJetonImpl` refuse de démarrer si le secret est absent ou fait moins de 32 octets. Le passage à RS256/ES256 (paire de clés asymétrique, rotation de clé) reste la cible recommandée par ce document et est à planifier avant la mise en production (voir la checklist §11) — ce n'est pas fait en V1 pour limiter la complexité opérationnelle du prototype (gestion de paire de clés, JWKS) tant qu'un HSM/gestionnaire de secrets n'est pas en place. |
