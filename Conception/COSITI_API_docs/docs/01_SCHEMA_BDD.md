...
Codes attendus en V1 (les 8 acteurs de `../Roles des acteurs.md`, aucun autre) : `PCA`, `DG`, `DGA`, `DAF`, `GESTIONNAIRE_COMPTES`, `CHEF_AGENTS_TERRAIN`, `AGENT_TERRAIN`, `ADMIN_SYSTEME`. `CHEF_AGENTS_TERRAIN` n'est pas un rôle organisationnel autonome : c'est un rôle additionnel attribué à un utilisateur qui porte déjà `AGENT_TERRAIN`, via désignation par la DGA (voir `agent.chef_designe_le` §2 et `historique_designation_chef` §2). Ne jamais recréer `RESP_CNPS`, `RESP_ZONE`, `TELECONSEILLER` ou `MARKETING` — rôles hors périmètre V1 (`../Roles des acteurs.md §16`).
...
| objectif_collecte_mensuel | NUMERIC(14,2) | nullable — besoin `[A]` à confirmer |
| actif | BOOLEAN | NOT NULL DEFAULT true |
| cree_par_dga_id | UUID | FK → utilisateur, nullable — trace la DGA à l'origine de la création (`Roles des acteurs.md §13.4`) |
| est_chef | BOOLEAN | NOT NULL DEFAULT false `[A]` — vrai si l'agent porte actuellement la désignation de Chef |
| chef_designe_le | TIMESTAMPTZ | nullable `[A]` |
| chef_designe_par | UUID | FK → utilisateur, nullable `[A]` — doit porter le rôle `DGA` |

Contrainte : un seul agent avec `est_chef = true` par périmètre organisationnel — règle exacte du périmètre à confirmer (`[A]`, `Roles des acteurs.md §14`).

### `historique_designation_chef` `[A]`
Trace chaque désignation et remplacement du Chef des agents de terrain — exigé par `Roles des acteurs.md §13.3` (« action métier explicite et auditée ») et `REC-H02`/`REC-H03`. Contrat à valider avant codage (jalon J3).

`id` UUID PK · `agent_id` UUID FK NOT NULL (l'agent désigné Chef) · `agent_remplace_id` UUID FK nullable (ancien Chef, si remplacement) · `designe_par` UUID FK → utilisateur NOT NULL (doit porter le rôle `DGA`) · `motif` VARCHAR(200) · `horodatage` TIMESTAMPTZ NOT NULL DEFAULT now().

### `compte_rendu` `[A]`
Structure la chaîne hiérarchique terrain (`Roles des acteurs.md §12.1`) : Agent de terrain → Gestionnaire des comptes → DGA. Contrat à valider avant codage (jalon J8, voir `Roles des acteurs.md §14`).

`id` UUID PK · `type_flux` VARCHAR(30) CHECK IN (`AGENT_VERS_GESTIONNAIRE`, `GESTIONNAIRE_VERS_DGA`) · `auteur_id` UUID FK → utilisateur NOT NULL · `destinataire_id` UUID FK → utilisateur, nullable · `periode_debut` DATE · `periode_fin` DATE · `contenu` JSONB NOT NULL (compte rendu structuré, pas de texte libre non structuré) · `comptes_rendus_sources` UUID[] nullable (pour un compte rendu `GESTIONNAIRE_VERS_DGA`, référence les comptes rendus agents consolidés) · `statut` VARCHAR(20) CHECK IN (`BROUILLON`, `CONTROLE`, `CONSOLIDE`, `TRANSMIS`) · `date_transmission` TIMESTAMPTZ nullable · champs d'audit.

### `rapport_daf` `[A]`
Structure le flux DAF → PCA (`Roles des acteurs.md §6`, `§12.3`). Contrat à valider avant codage (jalon J5/J10).

`id` UUID PK · `periode_debut` DATE · `periode_fin` DATE · `contenu` JSONB NOT NULL · `document_id` UUID FK → document, nullable (version imprimable) · `statut` VARCHAR(20) CHECK IN (`BROUILLON`, `PRODUIT`, `TRANSMIS`) · `produit_par` UUID FK → utilisateur NOT NULL · `transmis_le` TIMESTAMPTZ nullable · champs d'audit.
...
| statut | VARCHAR(20) | `BROUILLON`, `A_CONTROLER`, `VALIDE`, `RAPPROCHE`, `ANNULE` |
...
Règle : un agent ne valide jamais sa propre remise.
...
Ne pas développer au-delà de la traçabilité de base tant que le DAF n'a pas spécifié le module.
...
`ADHERENT_CREATION`, `ADHERENT_MODIFICATION`, `ADHERENT_ARCHIVAGE`, `ADHERENT_CHANGEMENT_STATUT`, `ADHERENT_DOUBLON_IGNORE`, `AGENT_CREATION_PAR_DGA`, `AGENT_DESIGNATION_CHEF`, `AGENT_REMPLACEMENT_CHEF`, `PORTEFEUILLE_AFFECTATION`, `PORTEFEUILLE_TRANSFERT`, `PAIEMENT_CREATION`, `PAIEMENT_VALIDATION`, `PAIEMENT_CORRECTION`, `PAIEMENT_ANNULATION`, `PAIEMENT_RAPPROCHEMENT`, `AFFECTATION_CREATION`, `DROITS_RECALCUL`, `REMISE_CAISSE_DECLARATION`, `REMISE_CAISSE_RECEPTION`, `REMISE_CAISSE_ECART`, `CNPS_DOSSIER_CREATION`, `CNPS_PIECE_AJOUT`, `CNPS_CHANGEMENT_STATUT`, `CNPS_DECLARATION_TRANSMISE`, `DOCUMENT_TELEVERSEMENT`, `DOCUMENT_CONSULTATION`, `DOCUMENT_SUPPRESSION_LOGIQUE`, `RELANCE_ENREGISTREMENT`, `COMPTE_RENDU_CREATION`, `COMPTE_RENDU_CONSOLIDATION`, `COMPTE_RENDU_TRANSMISSION`, `RAPPORT_DAF_PRODUCTION`, `RAPPORT_DAF_TRANSMISSION`, `UTILISATEUR_CREATION`, `UTILISATEUR_DESACTIVATION`, `ROLE_MODIFICATION`, `PERMISSION_MODIFICATION`, `PARAMETRE_MODIFICATION`, `CONNEXION_SUCCES`, `CONNEXION_ECHEC`, `EXPORT_SENSIBLE`, `ACCES_REFUSE`, `MIGRATION_IMPORT`.
